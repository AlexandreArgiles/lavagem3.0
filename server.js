...
app.get('/api/agenda', checkAuth, (req, res) => {
    db.all("SELECT a.*, c.nome as cliente, c.telefone, v.placa, v.modelo, s.nome as servico FROM agendamentos a JOIN clientes c ON a.cliente_id = c.id JOIN veiculos v ON a.veiculo_id = v.id JOIN servicos s ON a.servico_id = s.id WHERE a.status = 'PENDENTE' ORDER BY a.data_agendada ASC", (err, rows) => res.json(rows||[]));
});
